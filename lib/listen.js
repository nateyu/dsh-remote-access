...
import { isHtmlDocument, rewriteHtmlDocument, rewriteUpgradeHeaders, rewriteUpstreamHeaders, sanitizeDownstreamHeaders } from './page-patch.js'
...
export function createProxyServer(options) {
  ...
  function proxyHttp(req, res) {
    const path = pathnameOf(req)
    const document = path === '/' || path === '/index.html'
    ...
    const upstream = httpRequest({
      ...
      const html = isHtmlDocument(path, req.headers)
        && String(incoming.headers['content-type'] ?? '').includes('text/html')
        && !isCompressed(incoming.headers)
      if (!html) {
        res.writeHead(incoming.statusCode ?? 502, sanitizeDownstreamHeaders(incoming.headers))
        incoming.pipe(res)
        return
      }
      /** @type {Buffer[]} */
      ...
      incoming.on('end', () => {
        ...
        const out = sanitizeDownstreamHeaders(incoming.headers, { dropContentLength: true })
